...
from pymcfs.skeleton import contract_mesh
...
def test_contract_mesh_basic():
    mesh = tm.creation.icosphere(subdivisions=2, radius=1.0)
    Vt, Ft = contract_mesh(mesh, max_iterations=15, medial_weight=0.0)
...
def test_contract_mesh_medial_runs():
    mesh = tm.creation.icosphere(subdivisions=1, radius=1.0)
    Vt, Ft = contract_mesh(mesh, max_iterations=10, profile="starlab")
    assert Vt.shape[0] > 0
    assert _no_degenerate_faces(Ft)
