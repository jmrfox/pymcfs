...
def test_resolve_mcfs_profile_defaults():
    wh, wm, gate = resolve_mcfs_profile(
        None, attraction_weight=0.5, medial_weight=5.0, gate_exterior_poles=None
    )
    assert (wh, wm, gate) == (0.5, 5.0, True)

    wh, wm, gate = resolve_mcfs_profile(
        "starlab", attraction_weight=0.5, medial_weight=5.0, gate_exterior_poles=None
    )
    assert (wh, wm, gate) == (0.1, 0.2, False)

    wh, wm, gate = resolve_mcfs_profile(
        "starlab", attraction_weight=0.3, medial_weight=1.0, gate_exterior_poles=True
    )
...
def test_ts1_robust_no_screenshot_exterior_spikes():
    ...
    if not TS1.is_file():
        pytest.skip("toric_spines/data/mesh/TS1.obj not present")
