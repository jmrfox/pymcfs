...
Pass `use_cholmod=True|False|None` on `skeletonize` / `thin_mesh` / the driver
(`None` = use CHOLMOD when importable).

!!! note "Nearly singular warning"
    CHOLMOD may report `rcond ~ 1e-15`. Pinned vertices use
    `w_H = 1/zero_TH` (≈`1e7`), so `AᵀA` diagonals reach `1e14`. Expected
    conditioning noise, not a failed solve.
...
!!! warning "Embree / float32"
    ...
    unit-ish meshes near the origin. TS neuron coordinates (thousands of µm)
    need the default exact float64 gating path.
